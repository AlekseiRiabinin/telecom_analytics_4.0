...
def debug_task():
    logger.info("DEBUG: CSV to MSSQL DAG is executing!")
...
with DAG(
    ...
    description='Load CSV from MinIO to MSSQL SalesData table',
    schedule_interval=None,
    catchup=False,
    tags=['csv', 'minio', 'mssql', 'salesdata'],
    max_active_runs=1,
) as dag:
    ...
    csv_to_mssql = SparkSubmitOperator(
        task_id='csv_minio_to_mssql_job',
        application='/opt/airflow/dags/spark/pipelines/telecom_etl/jobs/minio_to_mssql_csv.py',
        name='csv-minio-to-mssql',
    ...
    start >> debug >> check_conn >> csv_to_mssql >> end
